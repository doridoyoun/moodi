/**
 * Mood persisted state: normalization + migration from older schemas into the current unified shape.
 * Source of truth on disk: @see ../storage/mood/moodStorage.js (moodi_unified_state_v2).
 * Legacy keys (v1, split timeline/gallery) are read only for one-time upgrade paths.
 */

import AsyncStorage from '@react-native-async-storage/async-storage';
/** @deprecated Legacy split storage — imported only for migration fallback. */
import { loadGalleryState } from '../../storage/galleryStateStorage';
import {
  loadTimelineByDate,
 ...
 */
export function normalizeEmotionToastByDate(raw) {
...
export function safeAlbumItems(arr) {
...
async function readLegacyInnerFrameKey() {
  let frameKey = 'white';
  try {
...
/**
 * Full load + migration pipeline. Returns current unified schema (entries + albumItems + galleryByDate + emotionToastByDate).
 */
export async function migrateLoadMoodState() {
  try {
    ...
    if (rawV2) {
      ...
      if (!hasPerDay || Object.keys(galleryByDate).length === 0) {
        ...
        const frameKey = await readLegacyInnerFrameKey();
        galleryByDate = {
          ...galleryByDate,
          [todayKey]: {
